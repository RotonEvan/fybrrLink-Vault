...
Graph::Graph(int m_nVertexNum, vector<vector<double>> edges) {
	clear();
	int start_vertex, end_vertex;
	double edge_weight;

	for(int i = 0;i < (int)edges.size();i++){
		start_vertex = edges.at(i).at(0);
		end_vertex = edges.at(i).at(1);
		edge_weight = edges.at(i).at(2);
...
void Graph::_import_from_file( const string& input_file_name )
{
	...
	{
		cerr << "start_vertex " << start_vertex << endl;
		if (start_vertex == -1)
		{
			break;
		}
...
BaseVertex* Graph::get_vertex( int node_id )
{
	...
	}else
	{
		...
		{
			// int vertex_id = (int)m_vtVertices.size();
