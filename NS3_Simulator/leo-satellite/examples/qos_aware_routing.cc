...
int main (int argc, char *argv[]) {
    ...
    double maxCD, deltaCD;
    int no_of_iter;
    bool isSuitablePathFound;
    int count = 0;
    ...
    for(int flow_index = 0; flow_index < N;flow_index++){
      ...
      std::cout<<"["<<flow_index+1<<"/"<<N<<"] : Printing Bresenhem Path Coordinates";
      ...
      if (!qosEnabled.at(flow_index)){
      ...
      else if(sat_network.isSatisfying(path_BW, path_latency, path_PLR, flowParams.at(flow_index).at(0), flowParams.at(flow_index).at(1), flowParams.at(flow_index).at(2))){
        sat_network.updateGraph(edge_index, flowParams.at(flow_index).at(0));
        std::cout<<"["<<flow_index+1<<"/"<<N<<"] : Flow successfully routed"<<std::endl;
        count++;
      }
        ...
        maxCD = 0.82;
        deltaCD = 0.05;
        no_of_iter = (int)((0.97 - maxCD)/deltaCD);
        isSuitablePathFound = false;

        adjList = sat_network.getSubGraph(x1, y1, x2, y2);
        
        for (int iter = 0;iter < no_of_iter; iter++){
            std::cout<<"["<<flow_index+1<<"/"<<N<<"] : Running Dijkstra Iteration no. "<<iter+1<<std::endl;
            path = sat_network.DijkstraAlgorithm(source_access_id, destination_access_id, adjList, true, maxCD);
            std::tie(edge_index, path_BW, path_latency, path_PLR) = sat_network.getPathParameters(path);
